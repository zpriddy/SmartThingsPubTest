...
definition(
    name: "1-TCP-Bulbs-Service-Manager-V2",
    namespace: "wackford",
    author: "Todd Wackford",
    description: "TCP Bulbs with current power consumption tile/reporting",
    category: "My Apps",
...
def initialize() {
    schedule("0 0/5 * * * ?", updateGatewayIP)
	if (selectedBulbs) {
		addBulbs()
	}
}
...
def locationHandler(evt) {
	...
	if (parsedEvent.ssdpTerm?.contains("urn:greenwavereality-com:service:gop:1"))
	{
        ...
        log.debug "parsedEvent.ip = $parsedEvent.ip"
        //log.debug "Gateway State Data: "

	}

	if (parsedEvent.headers && parsedEvent.body)
	{
    	log.debug( "in headers + body" )
